...
func GetCmdMint(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgMint(storeutils.NewEntityIDFromString(args[0]), ctx.GetFromAddress(), sdk.NewUintFromString(args[1]))
...
func GetCmdBurn(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgBurn(storeutils.NewEntityIDFromString(args[0]), ctx.GetFromAddress(), sdk.NewUintFromString(args[1]))
			return cliutil.ValidateAndBroadcast(ctx, bldr, msg)
		},
	}
}
...
func GetCmdTransfer(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgTransfer(storeutils.NewEntityIDFromString(args[1]), ctx.GetFromAddress(), toAddr, sdk.NewUintFromString(args[2]))
			return cliutil.ValidateAndBroadcast(ctx, bldr, msg)
		},
	}
}
