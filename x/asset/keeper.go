...
func (k Keeper) Create(ctx sdk.Context, name string, symbol string, owner sdk.AccAddress, totalSupply sdk.Uint) (types.Asset, sdk.Error) {
	...
	err := errs.WrapOrNil(storeutils.SetNotExists(ctx, k.storeKey, k.cdc, assetKey(id), asset))
	return asset, err
}
...
func (k Keeper) Inject(ctx sdk.Context, asset types.Asset) {
	...
	k.incrementSeq(ctx)
	if err := storeutils.SetNotExists(ctx, k.storeKey, k.cdc, assetKey(asset.ID), asset); err != nil {
		panic(err)
	}
}

func (k Keeper) Set(ctx sdk.Context, asset types.Asset) sdk.Error {
	return errs.WrapNotFound(storeutils.SetExists(ctx, k.storeKey, k.cdc, assetKey(asset.ID), asset))
}

func (k Keeper) Get(ctx sdk.Context, id storeutils.EntityID) (types.Asset, sdk.Error) {
	var a types.Asset
	err := errs.WrapNotFound(storeutils.Get(ctx, k.storeKey, k.cdc, assetKey(id), &a))
	return a, err
}

func (k Keeper) Has(ctx sdk.Context, id storeutils.EntityID) bool {
	return storeutils.Has(ctx, k.storeKey, assetKey(id))
}
...
func (k Keeper) Mint(ctx sdk.Context, id storeutils.EntityID, quantity sdk.Uint) sdk.Error {
...
func (k Keeper) Burn(ctx sdk.Context, id storeutils.EntityID, burner sdk.AccAddress, quantity sdk.Uint) sdk.Error {
	asset, err := k.Get(ctx, id)
	if err != nil {
		return err
	}
...
func (k Keeper) Transfer(ctx sdk.Context, id storeutils.EntityID, from sdk.AccAddress, to sdk.AccAddress, quantity sdk.Uint) sdk.Error {
	asset, err := k.Get(ctx, id)
	if err != nil {
		return err
	}
...
func (k Keeper) TransferFromOwner(ctx sdk.Context, id storeutils.EntityID, to sdk.AccAddress, quantity sdk.Uint) sdk.Error {
	asset, err := k.Get(ctx, id)
	if err != nil {
		return err
	}
...
func (k Keeper) Balance(ctx sdk.Context, id storeutils.EntityID, owner sdk.AccAddress) sdk.Uint {
	coins := k.bankKeeper.GetCoins(ctx, owner)
	out := coins.AmountOf(store.FormatDenom(id))
	return sdk.NewUintFromBigInt(out.BigInt())
}

func (k Keeper) incrementSeq(ctx sdk.Context) storeutils.EntityID {
	assetNum := storeutils.IncrementSeq(ctx, k.storeKey, []byte(seqKey))
	return assetNum
}

func assetKey(id storeutils.EntityID) []byte {
