...
func (k Keeper) Post(ctx sdk.Context, owner sdk.AccAddress, mktID storeutils.EntityID, direction matcheng.Direction, price sdk.Uint, quantity sdk.Uint, tif uint16) (types3.Order, sdk.Error) {
	var err sdk.Error
...
func (k Keeper) Create(ctx sdk.Context, owner sdk.AccAddress, marketID storeutils.EntityID, direction matcheng.Direction, price sdk.Uint, quantity sdk.Uint, tif uint16) (types3.Order, sdk.Error) {
	...
	err := storeutils.SetNotExists(ctx, k.storeKey, k.cdc, orderKey(id), order)
...
func (k Keeper) Cancel(ctx sdk.Context, id storeutils.EntityID) sdk.Error {
	var err sdk.Error
...
func (k Keeper) Get(ctx sdk.Context, id storeutils.EntityID) (types3.Order, sdk.Error) {
	var out types3.Order
	err := storeutils.Get(ctx, k.storeKey, k.cdc, orderKey(id), &out)
	return out, errs.WrapNotFound(err)
}

func (k Keeper) Set(ctx sdk.Context, order types3.Order) sdk.Error {
	return errs.WrapOrNil(storeutils.SetExists(ctx, k.storeKey, k.cdc, orderKey(order.ID), order))
}

func (k Keeper) Has(ctx sdk.Context, id storeutils.EntityID) bool {
	return storeutils.Has(ctx, k.storeKey, orderKey(id))
}

func (k Keeper) Del(ctx sdk.Context, id storeutils.EntityID) sdk.Error {
	return errs.WrapNotFound(storeutils.Del(ctx, k.storeKey, orderKey(id)))
}

func (k Keeper) incrementSeq(ctx sdk.Context) storeutils.EntityID {
	return storeutils.IncrementSeq(ctx, k.storeKey, []byte(seqKey))
}
