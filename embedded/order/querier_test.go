...
func TestQuerier(t *testing.T) {
	...
	t.Run("should return no more than 50 orders in descending order", func(t *testing.T) {
		...
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(55), res.Orders[0].ID)
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(6), res.Orders[49].ID)
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(5), res.NextID)
	})
	t.Run("should work with an offset", func(t *testing.T) {
		...
		res, err := doListQuery(ListQueryRequest{
			Start: storeutils.NewEntityID(7),
		})
		require.NoError(t, err)

		assert.Equal(t, 7, len(res.Orders))
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(7), res.Orders[0].ID)
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(1), res.Orders[6].ID)
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(0), res.NextID)
	})
	t.Run("should support filter by address alongside offset", func(t *testing.T) {
		...
		res, err := doListQuery(ListQueryRequest{
			Start: storeutils.NewEntityID(104),
			Owner: genOwner,
		})
		require.NoError(t, err)

		assert.Equal(t, 50, len(res.Orders))
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(109), res.Orders[0].ID)
		testutil.AssertEqualEntityIDs(t, storeutils.NewEntityID(11), res.Orders[49].ID)
	})
